...
import 'package:tot_nghiep_ban_sach_thu_vien/page_home/ItemDetailsSuccessOnline.dart';
...
class _BooksOnlinePageState extends State<BooksOnlinePage> {
  List<ItemProductSuccess> products = [];
  ...
  Map<String, ItemProductSuccess> productsMap = {};
    ...
    while (retryCount > 0) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ItemProductSuccess product = ItemProductSuccess.fromJson(data);
  ...
  Future<void> _fetchCartItems() async {
    try {
      ...
      List<ItemProductSuccess> cartItems = [];
      ...
      while (hasMoreData) {
        ...
        if (response.statusCode == 200) {
          ...
          if (jsonData['status'] == true) {
            ...
            if (product.success == 1 && product.pdf_file.isNotEmpty) {
              print(
                  "===================================Các Sách Online Hợp Lệ===================================");
              print("id: ${product.id}");
              ...
              if (productsMap.containsKey(product.productName)) {
                ...
                print(
                    'Sản phẩm từ fetchProducts: id=${itemProduct.id}, name=${itemProduct.productName}, price=${itemProduct.price}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Sách Online Của Bạn'),
      ),
      body: isFetching
  ...
  Widget _buildCard(ItemProductSuccess product) {
      ...
      child: InkWell(
        onTap: () {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => ItemDetailsSuccessOnlinePage(
